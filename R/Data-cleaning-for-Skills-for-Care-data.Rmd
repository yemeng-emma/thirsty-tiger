...
cqc_skills <- read_excel("data/SfC_CQC.xlsx") # CQC data with bridging column of SfC establishment id curated by Skills for Care
skills_raw <- read_csv("data/Skills_raw.csv") # raw Skills for Care organizational data
```
...
This results show that some facilities have more than 1 rating records. In detail the 
...
![capacity meaning](data/capacity.png)

- Main groups and corresponding values 

![main groups values](data/maingroups.png)
...
```{r}
table_main_group2 <- datasummary(main_group2 ~ form,
            data = merged_wide, fmt = 0)
table_main_group2
```
